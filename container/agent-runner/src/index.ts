...
async function runQuery(
  ...
  // FU-27a fold #3: inject runtime model identity into system prompt with
  // authoritative override language. Without this, the agent's "what model
  // are you?" answer is influenced by conversation history — and after a
  // /model switch, recent assistant turns were generated by a different
  // model, biasing self-report. This section is appended AFTER any
  // group-level CLAUDE.md so it has higher priority in the system prompt.
  const modelId = process.env.DAYSTROM_AGENT_MODEL;
  const modelIdentitySection = modelId
    ? `\n\n# Active Model Identity (runtime, authoritative)\n\nYou are running on model: \`${modelId}\`.\n\nWhen asked "what model are you?", "what model am I using?", or similar, ALWAYS report this exact identifier. The user can switch models mid-conversation via the \`/model\` command, so prior assistant turns in the conversation history may have been generated by a different model identity — that history is NOT authoritative for your current identity. The runtime value above is the only authoritative source.\n`
    : '';
  const systemPromptAppend = (globalClaudeMd || '') + modelIdentitySection;
