...
```
mcp__qmd__query {"searches":[{"type":"vec","query":"<JT query>"}],"rerank":false,"collections":["general"],"intent":"<one line of context>"}
```

<!-- 2026-09-12: the only MCP search tool is mcp__qmd__query. rerank:false is the default on the /research
     sync path; rerank:true is CPU-bound (47s–474s cold) and must NOT be used as a default on any path.
     Recipes: container/skills/qmd/SKILL.md. -->

`general` namespace covers prior `/research` reports (type: `research-report`), wiki pages (type: `wiki-page`), image-capture companions (type: `image-capture`), learning entries, brainstorm artifacts, and project notes. Single call surfaces them all. If results are sparse, add a `{"type":"lex","query":"<exact terms>"}` sub-query to the same call, and reshape the question toward the concept once before concluding the vault has nothing. Set `"rerank":true` only if JT explicitly requests a deeper search and accepts the wait. The `private` namespace is structurally unreachable from this container — do not attempt.
