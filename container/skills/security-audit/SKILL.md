...
Cross-check `container-inspect.json`: for each running container, check `.Config.Env` list. For each entry whose key (the part before `=`) matches `*_TOKEN` or `*_KEY` (case-insensitive), apply the two-step procedure below. **This procedure is binding — apply it literally; do not collapse the allowlist into the FAIL case.**

**Step A — read the allowlist table.**
...
- **IF** the matched key appears in the allowlist table **AND** the container's group folder is in the row's "Allowed in group(s)" column (`any` matches every group; otherwise the group folder must equal the listed value verbatim) → emit:

  `WARN: env var '<KEY>' present in container '<name>' — allowlisted per <Binding spec reference verbatim>`

- **OTHERWISE** → emit:

  `FAIL: credential key '<KEY>' leaked into container env for '<name>'`

The two forms are mutually exclusive: every match produces exactly one line, never both. Allowlisted matches MUST emit `WARN:` (never `PASS:`) so the architectural exception remains visible in every audit. Non-allowlisted matches MUST emit `FAIL:` regardless of value contents — the security guard does not look at values.

Future allowlist expansion: append a new row to the table with the same three-column shape (Key, Allowed in group(s), Binding spec reference). No other change to Step A or Step B logic.
