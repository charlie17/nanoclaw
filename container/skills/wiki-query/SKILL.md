...
2. **Primary search:** `mcp__qmd__query "<question>" -c wiki` — scoped to the `wiki` sub-collection (`general/wiki/`). Read the surfaced pages in full.
3. **Secondary search:** `mcp__qmd__query "<question>" -c general` — cross-reference pass over the full `general` namespace. Use results for connection surfacing only — research notes, learning entries, brainstorm artifacts, and project notes that carry query-relevant value the wiki doesn't yet cover. This is supplementary, not primary synthesis.
