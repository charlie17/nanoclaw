...
<!-- 2026-09-12: /wiki-lint runs unattended on the nightly cron (`0 6 * * *` UTC) — an automated path. Every lookup is `mcp__qmd__query` with a vec (or lex for exact terms) sub-query, `"rerank":false`, `"collections":["wiki"]` (or `["general"]` for cross-vault neighbors). `"rerank":true` is CPU-bound (47s–474s, GPU: none) and must NEVER be used on this automated path. Recipes: container/skills/qmd/SKILL.md. -->

## Audit dimensions (Karpathy line 66)

Sixteen dimensions audited in order:

1. **Orphan pages** — pages in `wiki/` with no inbound `[[wikilinks]]` from other wiki pages. **AUTO-FIX:** find 2-3 related concept pages (read `!index.md` + use a `mcp__qmd__query` vec sub-query, `rerank:false`, `collections:["wiki"]`, for semantic neighbors), add `[[wikilinks]]` from those pages to the orphan, and add return links from the orphan.
