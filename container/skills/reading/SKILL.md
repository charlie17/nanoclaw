...
- **Provenance stays legible forever.** Source content and JT overlay never mix: overlay lands below the `— JT —` divider line at the bottom of a card; stance recolors the node; source cards stay neutral. The legend card walks the whole workflow and every convention.
...
- **Root card** (labels allowed here only): central question, thesis, structure, scope. **Chapter hub cards:** chapter title + 2–3 sentence gloss (same no-attribution rules). **Legend card:** the four-step workflow (triage → arm → read → refresh) with the flag, stance, and color keys woven in — plain language, no typographic jargon.
